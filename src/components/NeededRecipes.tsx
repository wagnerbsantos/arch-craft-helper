  ...
  selectedRecipes.forEach((recipe) => {
    ...
    if (foundIndex !== -1) {
      ...
      if (recipe.components.length === 0) {
        ...
        newList.set(recipe, neededTreeCalculator(recipe.components, ownedRecipes))
      }
    }
  })
  return newList
...
const neededBaseCalculator = (selectedRecipes: Recipe[], ownedRecipes: Recipe[]): Recipe[] => {
  var newList = []
  selectedRecipes.forEach((recipe) => {
    const foundIndex = ownedRecipes.findIndex((owned) => owned.id === recipe.id)
    if (foundIndex !== -1) {
      ...
      if (recipe.components.length === 0) {
        ...
        newList = neededBaseCalculator(recipe.components, ownedRecipes).concat(newList)
      }
    }
  })
  return newList
